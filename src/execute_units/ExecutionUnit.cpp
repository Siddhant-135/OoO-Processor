...
ExecutionUnit::ExecutionUnit(UnitType name, int latency, int RS_size, std::vector<int>& memory, bool verbose): memory(memory){ 
this->name=name;
this->latency=latency;
this->verbose = verbose;

if(name == UnitType::LOADSTORE){
    myRS = std::unique_ptr<RS>(std::make_unique<LoadStoreQueue>(RS_size, latency, 1, verbose));
} else {
    myRS = std::unique_ptr<RS>(std::make_unique<RS>(RS_size, latency, 1, verbose)); 
}
}
...
if(idx!=-1){
    ...
    if(name== UnitType::ADDER){//ADD, SUB, ADDI, // TO ADD: SLTI and SLT in ADDER
        ...
        if(!check_output_bound(temp_output)) {
            ...
            output = 0;
            // Removed direct unit print, Processor can handle it if needed
        } 
        else {
    ...
    if (verbose) std::cout<<"Im there, nothing ready out of pipeline yet!\n";
    if(myRS->PipelineCounter<latency){//see fix, divide latency by stages per instruction.
        loadToPipeline();
        }
    return result;
...
void ExecutionUnit::print() const {
    std::string unitName;
    switch (name) {
        case UnitType::ADDER: unitName = "ADDER"; break;
        case UnitType::MULTIPLIER: unitName = "MUL"; break;
        case UnitType::DIVIDER: unitName = "DIV"; break;
        case UnitType::LOADSTORE: unitName = "L/S"; break;
        case UnitType::BRANCH: unitName = "BR"; break;
        case UnitType::LOGIC: unitName = "LOGIC"; break;
    }
    std::cout << "--- " << unitName << " RS ---\n";
    myRS->print();
}
