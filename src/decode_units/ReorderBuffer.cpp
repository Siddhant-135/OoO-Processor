...
void ROB::rob_capture_results(std::vector <ExuResult> tags_values){
    ...
    for(size_t i=0;i<tags_values.size();i++){
        ...
        // The Processor handles ROB printing now if verbose is set
    }
    return;
}
...
void ROB::print() const {
    std::cout << "--- ROB (" << number_of_entries << " entries) ---\n";
    if (is_Empty()) return;
    int cur = oldest_entry_idx;
    for (int i = 0; i < number_of_entries; i++) {
        const ROBEntry& entry = ROB_Vector[cur];
        std::cout << " [" << cur << "] PC: " << entry.inst_pc
                  << " | Ready: " << (entry.ready_from_RS ? "Y" : "N")
                  << " | DestReg: x" << entry.dest_regId << " Val: " << entry.dest_regVal
                  << " | MemAddr: " << entry.dest_memAddr << " MemVal: " << entry.dest_memVal
                  << (entry.has_exception ? " [EXCEPTION]" : "") << "\n";
        cur = (cur + 1) % capacity;
    }
}
